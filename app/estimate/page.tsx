...
function EstimatePageInner() {
  ...
  const jobDateRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    function handleClickOutside(e: MouseEvent) {
      ...
      if (jobDateRef.current && !jobDateRef.current.contains(e.target as Node)) {
  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-[800px] px-4 py-8">
        ...
        {/* Job Date */}
        <div className="mt-3" ref={jobDateRef}>
          ...
          <div className="mt-1 flex items-center gap-2 max-w-[280px]">
            {showDatePicker ? (
              <input
                ...
                className="block w-full rounded-xl bg-white/[0.05] border border-white/[0.1] px-3 py-2.5 text-sm text-white placeholder:text-slate-500 focus:border-white/20 focus:outline-none focus:ring-1 focus:ring-white/10 [color-scheme:dark]"
              />
            ) : (
              <input
                type="text"
            ...
            {jobDate && (
              <button
                type="button"
                onClick={() => { setJobDate(""); setShowDatePicker(false); }}
